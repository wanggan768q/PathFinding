...
bool  FindNavPath(float* start, float* end, int jflag, float* &outarr, int &len)
{
	...
	try
	{
		...
		// 如果没找到起点
		if (!m_startRef) {
			std::cerr << "stageInfo:" << ", Could not find startPoly: " << m_startRef << ", from startPos(" << m_spos[0] << "," << m_spos[2] << ") to endPos(" << m_epos[0] << "," << m_epos[2] << ")" << std::endl;
			//return NULL;
		}
		...
		if (!m_endRef) {
			...
			if (!m_endRef) {
				std::cerr << "stageInfo:" << ", Could not find endPoly: " << m_endRef << ", from startPos(" << m_spos[0] << "," << m_spos[2] << ") to endPos(" << m_epos[0] << "," << m_epos[2] << ")" << std::endl;
				//return NULL;
			}
		}
...
bool  Raycast(float* start, float* end, int jflag, float* &outarr, float* &oEdgeDir)
{
	...
	try {
		...
		// 如果没找到
		if (!m_startRef) {
			std::cerr << "stageInfo:" << ", Could not find startPos:(" << m_spos[0] << "," << m_spos[2] << "), startPoly:" << m_startRef << std::endl;
			//return NULL;
		}
