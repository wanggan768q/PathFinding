...
dtQueryFilter::dtQueryFilter() : m_includeFlags(0xffffffff),
                                 m_excludeFlags(0)
{
    ...
    else
    {
        ...
        do
        {
            ...
            if (node->flags & DT_NODE_PARENT_DETACHED)
            {
                float t, normal[3];
                int m;
                status = raycast(node->id, node->pos, next->pos, m_query.filter, &t, normal, path + n, &m, maxPath - n, NULL);
                n += m;
...
dtStatus dtNavMeshQuery::raycast(dtPolyRef startRef, const float *startPos, const float *endPos,
                                 const dtQueryFilter *filter,
                                 float *t, float *hitNormal, dtPolyRef *path, int *pathCount, const int maxPath, float *edgeDir) const
{
    dtRaycastHit hit;
    hit.path = path;
    hit.maxPath = maxPath;
    ...
    *t = hit.t;
    ...
    if (pathCount)
    {
        *pathCount = hit.pathCount;
        ...
        if (edgeDir != NULL && hit.pathCount > 0)
        {
            const dtMeshTile *tile = NULL;
            const dtPoly *poly = NULL;
            this->m_nav->getTileAndPolyByRefUnsafe(*hit.path, &tile, &poly);
            assert(tile != NULL && poly != NULL);

            float vertex1[3], vertex2[3];

            int nextEdgeIndex = (hit.hitEdgeIndex + 1) % poly->vertCount;
            dtVcopy(vertex1, &tile->verts[poly->verts[hit.hitEdgeIndex] * 3]);
            dtVcopy(vertex2, &tile->verts[poly->verts[nextEdgeIndex] * 3]);

            dtVsub(edgeDir, vertex1, vertex2);
        }
    }
